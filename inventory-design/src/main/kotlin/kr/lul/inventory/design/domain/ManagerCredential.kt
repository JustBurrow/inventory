...
interface ManagerCredential {
    companion object {
        ...
        fun validateManager(manager: Manager) {
            if (0 >= manager.id)
                throw AttributeValidationException("not persisted $ATTR_MANAGER", ATTR_MANAGER, manager)
        }
        ...
        fun isValidPublicKey(publicKey: String): Boolean =
                publicKey.length in PUBLIC_KEY_MIN_LENGTH..PUBLIC_KEY_MAX_LENGTH &&
        ...
        fun validatePublicKey(publicKey: String) {
            val msg = if (PUBLIC_KEY_MIN_LENGTH > publicKey.length)
                "too short $ATTR_PUBLIC_KEY(min=$PUBLIC_KEY_MIN_LENGTH)"
            else if (PUBLIC_KEY_MAX_LENGTH < publicKey.length)
                "too long $ATTR_PUBLIC_KEY(max=$PUBLIC_KEY_MAX_LENGTH)"
            else if (!publicKey.matches(Manager.NAME_REGEX) && !publicKey.matches(Manager.EMAIL_REGEX))
                "illegal $ATTR_PUBLIC_KEY pattern(patterns=$PUBLIC_KEY_PATTERNS)"
            else
                null

            if (null != msg)
                throw AttributeValidationException(msg, ATTR_PUBLIC_KEY, publicKey)
        }
        ...
        fun validateSecretHash(secretHash: String) {
            if (!secretHash.matches(SECRET_HASH_REGEX))
                throw AttributeValidationException("illegal $ATTR_SECRET_HASH pattern(pattern='$SECRET_HASH_PATTERN')",
                        ATTR_SECRET_HASH, secretHash)
        }
    }
