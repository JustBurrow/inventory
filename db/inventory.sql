...
CREATE TABLE IF NOT EXISTS `mgr_noun` (
  ...
  INDEX `IDX_NOUN_LABEL` (`label` ASC),
  INDEX `IDX_NOUN_LABEL_CODE` (`label_code` ASC),
...
-- Table `log_noun`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `log_noun` (
...
CREATE  OR REPLACE VIEW `view_manager` AS
    ...
    FROM
        `mgr_manager` AS `m`
            LEFT JOIN
        `mgr_manager_credential` AS `c` ON `m`.`id` = `c`.`manager`
...
SHOW WARNINGS$$
USE `inventory`$$
CREATE DEFINER = CURRENT_USER TRIGGER `inventory`.`mgr_noun_update` AFTER UPDATE ON `mgr_noun` FOR EACH ROW
BEGIN
    INSERT INTO `inventory`.`log_noun` (`crud`, `id`, `manager`, `noun_type`, `noun_key`, `label`, `label_code`, `max_cnt`, `description`, `created_utc`, `updated_utc`, `modify_ts`)
    VALUES ('U', OLD.`id`, OLD.`manager`, OLD.`noun_type`, OLD.`noun_key`, OLD.`label`, OLD.`label_code`, OLD.`max_cnt`, OLD.`description`, OLD.`created_utc`, OLD.`updated_utc`, OLD.`modify_ts`);
END$$

SHOW WARNINGS$$
SHOW WARNINGS$$
USE `inventory`$$
CREATE DEFINER = CURRENT_USER TRIGGER `inventory`.`mgr_noun_delete` AFTER DELETE ON `mgr_noun` FOR EACH ROW
BEGIN
    INSERT INTO `inventory`.`log_noun` (`crud`, `id`, `manager`, `noun_type`, `noun_key`, `label`, `label_code`, `max_cnt`, `description`, `created_utc`, `updated_utc`, `modify_ts`)
    VALUES ('D', OLD.`id`, OLD.`manager`, OLD.`noun_type`, OLD.`noun_key`, OLD.`label`, OLD.`label_code`, OLD.`max_cnt`, OLD.`description`, OLD.`created_utc`, OLD.`updated_utc`, OLD.`modify_ts`);
END$$
...
-- -----------------------------------------------------
-- Data for table `mgr_manager`
-- -----------------------------------------------------
START TRANSACTION;
USE `inventory`;
INSERT INTO `mgr_manager` (`id`, `email`, `name`, `created_utc`, `updated_utc`, `modify_ts`) VALUES (DEFAULT, 'system@inventory', 'system', 0, 0, DEFAULT);
